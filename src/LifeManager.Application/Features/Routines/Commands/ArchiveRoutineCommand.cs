...
public record ArchiveRoutineCommand(Guid Id) : IRequest<RoutineDto>, IBaseCommand;

public class ArchiveRoutineCommandHandler : IRequestHandler<ArchiveRoutineCommand, RoutineDto>
{
    private readonly IUnitOfWork _uow;
    private readonly ICurrentUserService _currentUser;

    public ArchiveRoutineCommandHandler(IUnitOfWork uow, ICurrentUserService currentUser)
    {
        _uow = uow;
        _currentUser = currentUser;
    }

    public async Task<RoutineDto> Handle(ArchiveRoutineCommand request, CancellationToken ct)
    {
